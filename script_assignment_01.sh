...
#alter the file line
if [ "$View" == "BID" ]; then
if cat sig.conf | grep -n -v "vdopiasample" | grep -n -q "$COMPname" ; then
A=$( cat sig.conf | grep -n -v  "vdopiasample" | grep  "$COMPname" | awk  -F ':' '{print $1}')
B=$( cat sig.conf | grep -n -v  "vdopiasample" | grep  "$COMPname" | awk  -F ';' '{print $2}')
C=$( cat sig.conf | grep -n -v  "vdopiasample" | grep  "$COMPname" | awk  -F '=' '{print $2}')
...
if cat sig.conf | grep -n  "vdopiasample" | grep -n -q "$COMPname " ; then

A=$( cat sig.conf | grep -n   "vdopiasample" | grep  "$COMPname" | awk  -F ':' '{print $1}')
B=$( cat sig.conf | grep -n   "vdopiasample" | grep  "$COMPname" | awk  -F ';' '{print $2}')
C=$( cat sig.confi | grep -n   "vdopiasample" | grep   "$COMPname" | awk  -F '=' '{print $2}')
